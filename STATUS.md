...
| 2026-06-05 | binance BINANCE_USE_BBO=1 depth regression | При BBO=1 URL переключался на @bookTicker ТОЛЬКО. a.books[token] = nil → snapshot = 1 уровень вместо 20×20. bid<ask тест тривиально прошёл. bybit/okx/bitget были корректны (dual-track): binance — единственный outlier | Фикс: dual-track URL (depth20 + bookTicker оба), MaxSymbols 200→100 при useBBO. Деплой pending. |
| 2026-06-05 | gate/aster/htx/hyperliquid/bingx BBO depth regression | ВСЕ 5 адаптеров с активными BBO-флагами имели тот же баг: BBO mode подписывался ТОЛЬКО на BBO-канал. a.books nil → 1 уровень вместо 20×20. Скриншот пользователя подтвердил: Binance 1 уровень, OKX (dual-track) — полная лесенка. | Фикс: dual-track для всех 5 (aster: @depth20+@bookTicker; bingx: @depth20+@bookTicker; htx: depth.high_freq+market.bbo; hyperliquid: l2Book+bbo; gate: order_book_update+book_ticker). Коммит 2d7a66c. Все exchange-тесты ✓. **ДЕПЛОЙ PENDING** (SSH таймаут). |
...
| 2026-06-05 | **Позиции REST-фолбэк:** 3 изменения в trade_service.py. (1) Timeout 10s→5s за флагом AVALANT_POSITIONS_TIMEOUT_S. (2) Stale-while-revalidate окно 5min→30min — пользователи, вернувшиеся после 5-30мин, получают мгновенный ответ вместо 6-9с ожидания. (3) Мониторинг WS-лайвности: log INFO "positions REST fallback" когда ws_status!=LIVE — видно в логах какие кошельки/биржи триггерят медленный путь. Коммит f8bede7. **ДЕПЛОЙ PENDING** (SSH таймаут). | Ожидаемый эффект: worst-case 10→5с; "вернулся через 20мин" = мгновенно. Замер до/после деплоя: позиционирование через DevTools → Network → /trade/positions, timing. | Следующее: Фаза 3 stakan — resync seq gap Kraken/HTX, delta-unsubscribe. |
