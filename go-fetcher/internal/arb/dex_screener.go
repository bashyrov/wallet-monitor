...
// DEXScreenerCompute is the pre-OKX dex-short pipeline kept alive as a
// parallel mode: CoinGecko symbol→contract cache (1h TTL) + DexScreener
// pool prices, funding join from the store, writes
// dex_screener_arbitrage.json. Row shape is identical to DEXCompute's
// dex_arbitrage.json so the frontend render path is shared.
type DEXScreenerCompute struct {
	store    *funding.Store
	books    *cache.Store
	cacheDir string
...
func (c *DEXScreenerCompute) SetCexRegistry(m CexAddressMatcher) {
	c.mu.Lock()
	c.cexMatcher = m
	c.mu.Unlock()
}
...
func NewDEXScreenerCompute(store *funding.Store, books *cache.Store, cacheDir string, interval time.Duration) *DEXScreenerCompute {
	return &DEXScreenerCompute{
		store:     store,
		books:     books,
		cacheDir:  cacheDir,
		interval:  interval,
...
func (c *DEXScreenerCompute) Run(ctx context.Context) error {
...
func (c *DEXScreenerCompute) tick(ctx context.Context) {
	c.refreshCG(ctx)
	c.mu.Lock()
	...
	// Fetch in batches with bounded concurrency. Stays under 300 req/min
	...
	log.L().Info().
		...
		Msg("dex-screener cycle complete")
	...
	if cntBatchOK == 0 && len(dexBySym) == 0 {
		log.L().Warn().Msg("dex-screener cycle starved (all batches failed) — keeping prior file")
		return
	}
	...
	if err := writeAtomic(filepath.Join(c.cacheDir, "dex_screener_arbitrage.json"), out); err != nil {
		log.L().Warn().Err(err).Msg("dex_screener_arbitrage write failed")
	}
}

func (c *DEXScreenerCompute) writeEmpty() {
	out := map[string]any{
	...
	_ = writeAtomic(filepath.Join(c.cacheDir, "dex_screener_arbitrage.json"), out)
}

// refreshCG pulls top-1000 markets and the full coins-list with platforms,
// merges into cgCache. 1h TTL.
func (c *DEXScreenerCompute) refreshCG(ctx context.Context) {
	c.mu.Lock()
	...
	for _, coin := range coins {
		...
		for _, pref := range chainPreference {
			for cgChain, addr := range coin.Platforms {
	...
	c.mu.Unlock()
	log.L().Info().Int("symbols", len(newCache)).Msg("CG cache refreshed (dex-screener)")
}
