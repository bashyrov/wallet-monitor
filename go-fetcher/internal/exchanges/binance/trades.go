// trades.go — Binance USDT-perp aggregated-trade stream.
//
// Channel: <sym>@aggTrade — every aggregated trade (≈ every fill) pushed
// in real time. No throttling, no aggregation interval. Hot pairs like
// BTC/ETH emit 50-200 events/sec; mid-cap pairs 5-20/sec.
//
// Combined-stream URL: same shape as futures.go but with @aggTrade
// instead of @depth20@100ms. Mounted as a separate ws.Runner — one TCP
// connection per stream type per venue keeps the two feeds independent.
//
// Event wire (inside the combined-stream wrapper):
//
//	{"stream":"btcusdt@aggTrade",
//	 "data": {"e":"aggTrade","E":..,"T":..,"s":"BTCUSDT",
//	          "a":12345,"p":"0.001","q":"100","f":100,"l":105,"m":true}}
//
...
// Trades is the ticks.Adapter for Binance @aggTrade.
type Trades struct {
...
func (a *Trades) URL(_ context.Context) (string, error) {
	...
	a.mu.Unlock()
	if len(syms) == 0 {
		return tradesCombinedBase + "?streams=btcusdt@aggTrade", nil
	}
	parts := make([]string, len(syms))
	for i, s := range syms {
		parts[i] = strings.ToLower(s) + "usdt@aggTrade"
	}
...
func (a *Trades) BuildSubscribe(symbols []string) [][]byte {
	...
	for i := 0; i < len(listed); i += chunkSize {
		...
		for j, s := range listed[i:end] {
			params[j] = strings.ToLower(s) + "usdt@aggTrade"
		}
