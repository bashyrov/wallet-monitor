...
// ── BuildSubscribe uses level2Depth5 and XBT alias ───────────────────
...
func TestBuildSubscribe_TargetsDepth5(t *testing.T) {
	...
	if !strings.Contains(string(frames[0]), "/contractMarket/level2Depth5:XBTUSDTM") {
		t.Errorf("BTC → XBT alias + Depth5 missing: %s", frames[0])
	}
	if !strings.Contains(string(frames[1]), "/contractMarket/level2Depth5:ETHUSDTM") {
...
func TestParse_WrongTypeIgnored(t *testing.T) {
	...
	// wrong topic (level2: not level2Depth5:)
...
func TestParse_ETHSymbol(t *testing.T) {
	a := &Futures{auth: &authClient{}}
	frame := []byte(`{"type":"message","topic":"/contractMarket/level2Depth5:ETHUSDTM","data":{"bids":[["2000","1"]],"asks":[["2001","2"]],"ts":9999}}`)
	snap, err := a.Parse(frame)
	if err != nil || snap == nil {
		t.Fatal(err)
	}
