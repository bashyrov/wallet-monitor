...
_ARB_CACHE_TTL = 0.5  # event-driven engine refreshes faster than this; just a lower bound


# ══════════════════════════════════════════════════════════════════════════════
# Event-driven arb engine
#
# The periodic `_refresh_loop` (every 3s) was wasteful: 99% of its work was
# recomputing pairs whose inputs hadn't changed since the last tick. Live
# engine replaces that with a per-symbol pub/sub:
#
#   - Every WS/REST update calls `on_row_update(exchange, symbol)` which
#     enqueues the symbol on `_event_queue`.
#   - `arb_event_loop()` (single consumer) drains the queue with a 30ms
#     debounce window, then recomputes ONLY the pairs involving the pending
#     symbols. Updates `_live_opps` keyed by (symbol, long, short).
#   - Broadcaster (in screener.py) snapshots `_live_opps` every 250ms and
#     pushes diffs to WS clients.
#
# Complexity per tick: O(N²) where N = number of exchanges listing the symbol
# (usually 5-11). That's ~30-120 field comparisons per tick, trivial even at
# 1000 ticks/sec.
# ══════════════════════════════════════════════════════════════════════════════

_event_queue: "asyncio.Queue[str] | None" = None
_live_opps: dict[tuple, dict] = {}   # (symbol, long_exchange, short_exchange) → opp
_live_opps_lock = None                # asyncio.Lock, lazily created in the event loop
_live_opps_ts = 0.0                   # last update timestamp
_event_loop_task: "asyncio.Task | None" = None

# How long to coalesce events for the same (and other) symbols before running
# recompute. 30ms is below any human-perceptible latency but absorbs the
# burstiness of 11 WS adapters emitting simultaneously.
_EVENT_DEBOUNCE_S = 0.03


def on_row_update(exchange: str, symbol: str) -> None:
    """Public hook called from any WS / REST source when a row ticks.
    Non-blocking: drops the event if the queue is full (the consumer is
    already behind and will sweep everything soon)."""
    q = _event_queue
    if q is None:
        return
    try:
...
def _compute_pair(
    ...
    """Compute one arb opportunity from two funding rows.

    Returns None when the pair is not eligible (missing interval, no orderbook,
    invalid prices, or net<=0). The result dict matches the legacy
    _compute_arb_sync schema exactly so downstream diff logic, frontend, and
    file-cache consumers don't need to change.
    """
    ...
    lv_l = top_levels_fn(long_e["exchange"], symbol)
    lv_s = top_levels_fn(short_e["exchange"], symbol)
...
def _symbol_rows(symbol: str) -> list[dict]:
    """Snapshot of funding rows for one symbol across all exchanges.

    Reads directly from the WS manager's live adapter state so the event
    engine always sees the same data a human would see in `funding_ws.json`,
    not whatever _cache last happened to mirror. Falls back to _cache for
    exchanges whose WS isn't healthy yet.
    """
    out: list[dict] = []
    seen: set[str] = set()
    # Primary: WS manager adapter rows (freshest, stamped per tick).
    try:
        from backend.services.funding_ws import _manager as _ws_mgr
    except Exception:
        _ws_mgr = None
    if _ws_mgr is not None:
        for ex, adapter in getattr(_ws_mgr, "_adapters", {}).items():
            row = adapter._rows.get(symbol)
            if row and row.get("price") and row.get("rate") is not None:
                merged = dict(row)
                merged["exchange"] = ex
                out.append(merged)
                seen.add(ex)
    # Fallback: per-exchange _cache for any exchange not yet covered.
    for ex, (rows, _ts) in _cache.items():
        if ex in seen:
            continue
        for r in rows:
...
def compute_symbol_opps(symbol: str) -> dict[tuple, dict | None]:
    """Recompute all opps involving `symbol`. Returns a dict keyed by
    (symbol, long, short) mapping to the new opp dict, or None when the
    pair is no longer eligible (needs to be removed from `_live_opps`).
    """
    ...
    min_volume = admin_settings.get_arb_min_volume_usd()
    if hidden and symbol in hidden:
        return {}
    rows = [r for r in _symbol_rows(symbol) if r["exchange"] not in exclude]
    # Volume gate per side.
    def _ok(r: dict) -> bool:
        v = r.get("volume_usd")
        try:
            return v is not None and float(v) >= min_volume
        except (TypeError, ValueError):
            return False
    rows = [r for r in rows if _ok(r)]
    # Outlier guard (re-use median heuristic from _drop_price_outliers — but
    # scoped to this symbol, so just drop rows far from the median).
    if len(rows) >= _MIN_EX_FOR_MEDIAN:
        try:
            from statistics import median
            ...
            if med > 0:
                rows = [
                    r for r in rows
                    if r.get("price") and abs(float(r["price"]) - med) / med * 100 <= _PRICE_DROP_DEV_PCT
                ]
        except Exception:
            pass
    out: dict[tuple, dict | None] = {}
    # For every previously-known pair of THIS symbol, we must re-evaluate —
    # either keep/update or remove (mark None).
    known_keys = [k for k in _live_opps.keys() if k[0] == symbol]
    new_keys: set[tuple] = set()
    for i in range(len(rows)):
        for j in range(len(rows)):
            if i == j:
                continue
            opp = _compute_pair(rows[i], rows[j], symbol, top_levels)
            if opp:
                key = (symbol, rows[i]["exchange"], rows[j]["exchange"])
                out[key] = opp
                new_keys.add(key)
    # Anything in known_keys not in new_keys gets removed.
    for k in known_keys:
...
async def _arb_event_loop() -> None:
    """Consumer: coalesces symbol events and recomputes affected pairs.
    Runs forever until the fetcher shuts down."""
    global _event_queue, _live_opps_lock, _live_opps_ts
    _event_queue = asyncio.Queue(maxsize=10000)
    _live_opps_lock = asyncio.Lock()
    logger.info("arb event loop started (debounce=%.0fms)", _EVENT_DEBOUNCE_S * 1000)
    pending: set[str] = set()
    while True:
        sym = await _event_queue.get()
        pending.add(sym)
        # Drain additional events within the debounce window.
        deadline = asyncio.get_event_loop().time() + _EVENT_DEBOUNCE_S
        while True:
            remaining = deadline - asyncio.get_event_loop().time()
            if remaining <= 0:
                break
            try:
                extra = await asyncio.wait_for(_event_queue.get(), timeout=remaining)
                pending.add(extra)
            except asyncio.TimeoutError:
                break
        # Recompute each pending symbol. Compute is CPU-bound but trivial
        # (O(n²) per symbol, n ≤ 11) — no thread hop needed.
        to_process = list(pending)
        pending.clear()
        changes = 0
        for s in to_process:
            try:
                updates = compute_symbol_opps(s)
            except Exception as exc:
                logger.debug("arb event compute %s failed: %s", s, exc)
                continue
            async with _live_opps_lock:
                for key, opp in updates.items():
                    if opp is None:
                        if _live_opps.pop(key, None) is not None:
                            changes += 1
                    else:
                        prev = _live_opps.get(key)
                        if prev != opp:
                            _live_opps[key] = opp
                            changes += 1
        if changes:
            _live_opps_ts = time.time()
            # Publish snapshot to shared file cache for web broadcasters.
            # Coalesced: at most one write per event-loop batch, so bursts
            # don't thrash the tmpfs.
            try:
                snap = _live_opps_snapshot()
                _arb_result_cache["data"] = snap
                _arb_result_cache["ts"] = _live_opps_ts
                _write_file_cache("arbitrage.json", _slim_arb_for_file(snap))
            except Exception as exc:
                logger.debug("arb event loop snapshot write failed: %s", exc)


def _live_opps_snapshot() -> dict:
    """Return a {'ts', 'opportunities'} snapshot of the current live opps,
    sorted by net_profit desc. Cheap — just list(dict.values()) + sort."""
    # No lock needed: reads of dict snapshots are GIL-safe; downstream doesn't
    # mutate the list.
    opps = sorted(_live_opps.values(), key=lambda x: x.get("net_profit", 0), reverse=True)
    return {
        "ts":   int(_live_opps_ts or time.time()),
...
def start_arb_event_loop() -> None:
    """Start the event consumer (idempotent)."""
    global _event_loop_task
    if _event_loop_task and not _event_loop_task.done():
        return
    _event_loop_task = asyncio.create_task(_arb_event_loop(), name="arb_event_loop")


def stop_arb_event_loop() -> None:
    global _event_loop_task
    if _event_loop_task and not _event_loop_task.done():
