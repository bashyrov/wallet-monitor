...
from backend.domain.errors import TagNotFound, TagAlreadyExists, TagLimitReached
...
def list_tags(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return svc.list_tags(db, current_user.id)
...
def create_tag(body: TagCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        return svc.create_tag(db, body, current_user.id)
    except TagLimitReached as e:
...
def update_tag(tag_id: int, body: TagUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    _guard_system(db, tag_id)
    try:
        return svc.update_tag(db, tag_id, body, current_user.id)
...
def delete_tag(tag_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    _guard_system(db, tag_id)
    try:
        svc.delete_tag(db, tag_id, current_user.id)
    except TagNotFound as e:
        raise HTTPException(status_code=404, detail=str(e))
